...
async function main() {
  ...
  if (dbType === "sqlite") {
    const dbPath = customSqlitePath || process.env.SQLITE_PATH || path.join(ROOT_DIR, "data", "blogmbktech.db");
    ...
    if (!process.env.NEON_POSTGRES && !process.env.DATABASE_URL) {
      console.warn("⚠ PostgreSQL connection string not configured; skipping.");
    } else {
